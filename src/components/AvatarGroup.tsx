...
const AvatarGroup = () => {
    ...
    <RowComponent styles={{justifyContent:'flex-start'}}>
      {Array.from({length : 10}).map((item, index) => index < 3 && ( 
      <Image
      source={{uri: imageUrl}}
      key={`image${index}`}
    ...
    />
      ...
      {uidsLenght > 5 && (
        <View style={[imageStyle, {backgroundColor:'coral', justifyContent:'center',alignItems:'center', borderWidth:1, marginLeft:-10}]}>
        <TextComponent 
        flew={0}
        styles={{
          lineHeight:19,
